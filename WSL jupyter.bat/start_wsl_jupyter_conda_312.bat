...
echo 正在准备启动 WSL (Ubuntu-20.04)，激活 py312 conda 环境，并在当前目录的 WSL 映射路径中启动 Jupyter Notebook 服务器...
echo.
...
echo 将激活 py312 conda 环境并启动 Jupyter Lab。
...
start "WSL_Jupyter_Server" wsl -d Ubuntu-20.04 --cd "%WSL_PATH%" bash -lic "source /home/linyu/anaconda3/etc/profile.d/conda.sh ; conda activate py312 ; echo '--- Starting Jupyter Lab in py312 (Port: 8888) ---' ; /home/linyu/anaconda3/envs/py312/bin/jupyter lab --no-browser --ip=0.0.0.0 --notebook-dir=. ; echo '--- Jupyter Lab has exited. Press any key to close this WSL window. ---'; read -n 1 -s -r -p ''"
